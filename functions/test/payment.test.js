...
describe("payment", () => {
  ...
  describe("confirmAnticipation", () => {
    const genericTest = async (
      ...
      const wrapped = test.wrap(payment.confirm_anticipation);
      try {
        await wrapped(data, ctx);
        if (succeeed) {
          assert(true, "confirm_anticipation finished successfully");
        } else {
          assert(false, "confirm_anticipation didn't throw expected error");
        }
      } catch (e) {
        assert.strictEqual(e.code, expectedCode, "receive correct error code");
        assert.strictEqual(
    ...
    it("fails if 'anticipation_id' argument is missing", async () => {
      await genericTest(
        { pagarme_recipient_id: "pagarme_recipient_id" },
        "invalid-argument",
        "missing expected argument 'anticipation_id'."
      );
    });
    ...
    it("fails if 'recipient_id' argument has wrong type", async () => {
      await genericTest(
        {
          anticipation_id: 21,
          pagarme_recipient_id: "pagarme_recipient_id",
        ...
        "invalid-argument",
        "argument 'anticipation_id' has invalid type. Expected 'string'. Received 'number'."
      );
    });
  });
});
